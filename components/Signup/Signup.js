...
export default Signup = ({navigation}) => {
  ...
  const [NgaySinh_isDatePickerVisible, NgaySinh_setDatePickerVisibility] = React.useState(false);
  ...
  const register = async () => {
    const process = async () => {
      setLoading(true);
      try {
        const newImageUri = "file:///" + user['avatar'].uri.split("file:/").join("");
    ...
    <ScrollView style={{ marginTop: 20, marginBottom: 20 }}>
      ...
      <List.Section>
        ...
        <List.Item
          title={() => (
            <>
              <TextInput label="Ngày sinh" textColor="black" disabled={true} defaultValue={Moment(new Date()).format('YYYY-MM-DD')} value={Moment(NgaySinh_date).format('YYYY-MM-DD')}></TextInput>
          ...
          right={() => (
            ...
            <Button onPress={NgaySinh_showDatePicker} mode="contained-tonal"><List.Icon icon="calendar-check" /></Button>
          )}
        />
        <List.Subheader>
        ...
        <List.Item
          title={() => (
            <View style={styles.container}>
              {renderLabel()}
              <Dropdown
                style={[styles.dropdown, isFocus && { borderColor: "dimgray" }]}
        ...
        <List.Item
          title={() => (
            <>
              <TextInput label="Ngày nhập học" textColor="black" disabled={true} defaultValue={Moment(new Date()).format('YYYY-MM-DD')} value={Moment(date).format('YYYY-MM-DD')}></TextInput>
              {/* {date !== "" ? Moment(date).format('MM/DD/YYYY') : Moment(new Date()).format('MM/DD/YYYY')}</Text> */}
              <DateTimePickerModal
          ...
          right={() => (
            // <Button icon="calendar-check" mode="text" onPress={showDatePicker} />
            <Button onPress={showDatePicker} mode="contained-tonal"><List.Icon icon="calendar-check" /></Button>
          )}
        />
        <List.Item
          title={() => (
